...
class PostController {
     ...
     */
    static async listarPostPorFiltro(req: Request, res: Response, next: NextFunction) {
        try {
            const { titulo, descricao } = req.query;
            const busca: { [key: string]: any } = {};
            ...
            const postsDatasFormatadas = postsResultado.map((post: any) => {
     ...
     *         description: Erro interno do servidor
     */
    static async listarPosts(req:Request,res: Response, next: NextFunction) {
        ''
        try {
            const listaPostsAtivos = await PostModel.find({ postAtivo: true });

            const postsFormatados = listaPostsAtivos.map((post: any) => {
                const postObj = post.toObject();
     ...
     *         description: Erro interno do servidor
     */
    static async listarPostsProfessor(req:Request,res: Response, next: NextFunction) {
        try {
            const listaPosts = await PostModel.find({});

            const postsFormatados = listaPosts.map((post:any) => {
                const postObj = post.toObject();
                postObj.dataCriacao = post.dataCriacao.toLocaleDateString("pt-BR");
                postObj.dataAtualizacao = post.dataAtualizacao.toLocaleDateString("pt-BR");
                return postObj;
     ...
     */
    static async listarPostPorId(req: Request, res: Response, next: NextFunction) {
        try {
     ...
     */
    static async cadastrarPost(req: Request, res: Response, next: NextFunction) {
        try {
     ...
     */
    static async atualizarPost(req: Request, res: Response, next: NextFunction) {
        try {
            if (req.body.dataAtualizacao || req.body.dataCriacao) {
     ...
     */
    static async excluirPost(req: Request, res: Response, next: NextFunction) {
        try {
            const id = req.params.id;
