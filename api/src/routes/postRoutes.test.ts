...
jest.mock('../controllers/postController', () => ({
  PostController: {
    listarPosts: jest.fn((req: Request, res: Response) => res.status(200).json({ message: 'listarPosts' })),
    listarPostsProfessor: jest.fn((req: Request, res: Response) => res.status(200).json({ message: 'listarPostsProfessor' })),
    listarPostPorFiltro: jest.fn((req: Request, res: Response) => res.status(200).json({ message: 'listarPostPorFiltro' })),
    listarPostPorId: jest.fn((req: Request, res: Response) => res.status(200).json({ message: 'listarPostPorId' })),
    cadastrarPost: jest.fn((req: Request, res: Response) => res.status(201).json({ message: 'cadastrarPost' })),
    atualizarPost: jest.fn((req: Request, res: Response) => res.status(200).json({ message: 'atualizarPost' })),
    excluirPost: jest.fn((req: Request, res: Response) => res.status(204).end()),
  }
}));
